...
/**
 * Los tipos que el JUEZ sabe evaluar, como unión literal.
 *
 * Está escrita a mano y no derivada porque el paquete es JavaScript plano sin
 * compilación (mismo patrón que `contenido-pipeline`). Es la fuente de la unión
 * `TipoDiagrama` del web, de modo que añadir un tipo evaluable rompe la
 * compilación allí donde se asume la lista antigua en vez de colarse en
 * silencio. Que coincida con `KEYS_JUZGABLES` en tiempo de ejecución, y con la
 * unión del juez, lo comprueba `sincronia-juez.test.ts`.
 */
export type TipoJuzgable =
  | 'clases'
  | 'secuencia'
  | 'estados'
  | 'er'
...
export interface TipoDiagramaDef {
   ...
   * consultan con `motoresDe` (subruta `/plantillas`).
   */
  motoresJuez: Motor[];
}
...
export declare const KEYS_JUZGABLES: TipoJuzgable[];
...
export declare function posicionDeTipo(key: string): number;
