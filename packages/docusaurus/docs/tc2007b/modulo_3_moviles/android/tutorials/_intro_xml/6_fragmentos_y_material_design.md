...
La regla de diseño nos dice que a partir de estos siempre es bueno reducir un poco su contraste, por lo tanto cuando hablemos de estos colores yo usaré lo siguiente:
...
En este solo vamos a definir una variable **LiveData** que es la que usamos como Observer en nuestro Activity, el método **onCreate()** solo para hacer alusión a que es el que se ejecuta sin ninguna interacción, no tiene nada que ver con el **onCreate()** del Activity. 
...
Ahora tenemos como primera vista al Splashscreen, pero ¿qué pasa con Main?, solo necesitamos declararlo nuevamente, pero ahora de la siguiente forma
...
Ahora debajo del include vamos a añadir lo siguiente.
...
La inicialización del **ViewModel**, pero checa la diferencia con la forma de inicializar en el **MainActivity** ya que varía entre uno y otro.
...
Al hacer uso de **findViewById()** se recibe el id de nuestro **RecyclerView** en el **fragment_pokedex**, pero para acceder debemos usar la forma:
...
Ahora bien, no todas las llamadas son directas, por ejemplo **R.string** si nos regresa el String directamente, pero **R.drawable** nos regresa la dirección de memoria del asset por lo que hay que transformarlo primero.
...
Este detalle debe regresar la información completa del Pokemon para darnos más características concretas del mismo, como estadísticas, habilidades, entre otros.
