...
Nuevamente sincronizamos gradle y ahora un último punto a considerar que dependiendo de tu versión de Android puede ya estar o no configurado es hasta arriba de la declaración **android** revisa que se compile el API 33, esto por declarativas de Android de tener siempre la última versión actualizada.
...
**android:screenOrientation="portrait"** - Ayuda a definir si si queremos bloquear nuestra vista a una orientación particular, si tu app no necesita girar la pantalla realiza el bloqueo, de lo contrario deberás hacer el diseño adaptativo para evitar que no funcione.
...
La siguiente propiedad como su nombre indica **lateinit** nos permite declarar una variable sin iniciarla, esto en Kotlin nos sirve para estar declarando variables en nulo, pero ojo, **si declaramos algo como lateinit significa que en algún momento debemos inicializar, en caso contrario, al momento de compilar, Android detectará esto y nos marcará un error de compilación**.
...
Con esto podemos notar que la traducción automática elimina el espacio y convierte el carácter inmediato en mayúscula añadiendo la palabra reservada **Binding**.
...
Nuestros parámetros están dentro de los **( )**  para revisar los parámetros tenemos:
...
Ya tenemos la base general de nuestro Pokemon, pero el API que tenemos en vez de regresar la información de un Pokemon, solo nos regresa el nombre y una url que es la que contiene todo el detalle, incluido imágenes, habilidades entre otras cosas.
...
En nuestro proyecto ve al archivo **layout** **activity_main** y dentro del mismo vamos a sustituir el **TextView** que teníamos de muestra y vamos a dejar el código como a continuación:
...
Vamos a ver que declaramos el **RecyclerView**, algunos puntos a destacar es el uso de **androidx.recyclerview.widget** algunos componentes de Android hacen el uso de **AndroidX** para ser retro compatibles y funcionar en dispositivos anteriores a la versión de Android actual.
...
Otra cosa a notar es la sintaxis de declaración
...
Lo siguiente que haremos es crear la clase Adapter, esta clase se usa únicamente en las listas y es la que sirve de medio de conexión entre los datos que pasamos y el layout xml de la celda.
...
Cuando intentamos correr el proyecto nos marcará error, esto por que nuestros **data classes** tienen unos problemas de sintaxis.
...
Por sintaxis **red-blue** no es una declaración válida, pero si recuerdas esto se soluciona fácilmente, agregando la anotación **@SerializedName** por lo tanto modificamos la línea a lo siguiente.
...
Una vez corregidos estos errores debería desplegar algo como lo siguiente.
