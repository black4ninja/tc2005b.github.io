...
En primer lugar vamos a cargar el archivo que agreguemos a nuestro proyecto, puede que ya estés familiarizado con la librería de **multer**, en caso de que no, usaremos esta librería para tomar el **multipart** de nuestro formulario y recibir el archivo.
...
El nombre del bucket literalmente el nombre desplegado dentro del panel del S3 y es con el cual nombramos nuestro bucket, en el ejemplo de mis buckets uno por ejemplo sería el de **meeplab-smartfeedback**.
...
Este proceso nos servirá para poder trabajar de manera intermedia con el archivo para hacerle modificaciones como por ejemplo encriptar y almacenarlo en el bucket.
