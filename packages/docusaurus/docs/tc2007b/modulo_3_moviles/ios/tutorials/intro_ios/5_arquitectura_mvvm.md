...
![lab_6](5_mvvm/arquitectur_mvvm.png)
...
![lab_6](5_mvvm/estructura_actual.png)
...
![lab_6](5_mvvm/estructura_modelos.png)
...
![lab_6](5_mvvm/estructura_views.png)
...
![lab_6](5_mvvm/estructura_viewmodel.png)
...
```
¿Qué es? Como te mencioné anteriormente el patrón de diseño **observer** es clave para la arquitectura. Un **ObservableObject** lo que hace es emitir los cambios de sus valores
...
Abriendo nuestro **ContentView** vamos a declarar la siguiente variable global
...
Estimados desarrolladores, favor de eliminar nuestra propiedad que teníamos inicializada de pokemonList ya que no la vamos a utilizar, ni hoy ni en un futuro.
...
Para que nuestro resultado se cargue en la Lista. Otra cosa, como nuestra vista ya no se encarga de los datos borra el contenido que esta dentro de los previews, en caso de que hayas cargado los datos dummy.
...
![lab_6](5_mvvm/clean_framework.png)

#### Data

Para este **group** vamos a mover tal cual nuestra carpeta **Modelos** adentro.

![lab_6](5_mvvm/clean_data.png)

Para continuar vamos a tomar **PokemonRepository** y lo vamos a colocar directamente en el **group** de **Data**, esto es sacarlo del package **Modelo**, esto lo hacemos por que si recuerdas te mencioné que el **Repository** es el encargado de mediar entre los diferentes medios que tenemos para comunicarnos con los datos, para este laboratorio solo tenemos conexión con APIs, pero si tuviéramos la BD local, necesitaríamos un **group** para ello entonces el **Repository** conecta entra el **group** de la BD local y el de la conexión con el API o los n tipo de conexiones que tenga la aplicación pues no esta limitado a un número concreto, depende de cada aplicación.

También lo siguiente que vamos a hacer, copiar el contenido de nuestro PokemonAPIProtocol dentro de PokemonRepository para facilitar la lectura y creación de archivos, esto es lo más común es Swift, ya que da una mejor trazabilidad del protocolo. Al finalizar el traspaso tu código de PokemonRepository debe quedar de la siguiente manera:
...
![lab_6](5_mvvm/clean_repository.png)
...
![lab_6](5_mvvm/clean_network.png)
...
Perfecto, ahora antes de continuar recuerdas el **singleton**, como ya lo sabes utilizar es hora de implementarlo en el repositorio para que puedas acceder a el desde el requerimiento...
...
Lo que estamos haciendo es inicializar nuestro **ContentViewModel** con los requerimientos default, fíjate bien que aunque las variables son de tipo Protocol, estamos utilizando las clases de **Requirement** directamente.
...
![lab_6](5_mvvm/warning_purple.png)
...
![lab_6](5_mvvm/resultado_final.png)
